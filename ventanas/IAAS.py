...
def render():
    ...
    with c1:
        ...
        tipo_iaas = st.selectbox("Tipo de IAAS", lista_iaas, index=None)
        
        # Lógica condicional para OTRO
        especificar_otra = ""
        if tipo_iaas == "OTRO":
    ...
    for i in range(1, 5):
        with st.expander(f"Captura de Cirugía {i}"):
            ...
            with cols[0]:
                ...
                st.selectbox(f"Tipo {i}", ["Electiva", "Urgencia"], key=f"tipo_cir_{i}", index=None)
            with cols[1]:
                st.selectbox(f"Grado de contaminación {i}", ["Limpia", "Limpia con implante", "Limpia contaminada", "Contaminada", "Sucia"], key=f"grado_{i}", index=None)
