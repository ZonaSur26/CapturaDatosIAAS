...
def render():
    ...
    # --- FECHA CON CALENDARIO Y FORMATO dd/mm/aaaa ---
    ...
    with c_ed:
        edad_str = ""
        if f_nacimiento:
            # Mostramos al usuario el formato exacto que guardaremos
            fecha_formateada = f_nacimiento.strftime("%d/%m/%Y")
            st.info(f"Fecha: **{fecha_formateada}**")
            ...
            st.info("Seleccione una fecha en el calendario")
    ...
    if st.button("Guardar registro y continuar"):
        if not f_nacimiento or not entidad_nac or not sexo:
            ...
            # AQUÍ ES DONDE FORZAMOS EL FORMATO dd/mm/aaaa
