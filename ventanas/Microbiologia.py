...
def render():
    ...
    if (se_tomo_muestra == "Sí" or hemocultivo_its == "Sí") and hab_micro:
        ...
        with c1:
            ...
            laboratorio = st.selectbox("LABORATORIO", lab_opciones, key="k_lab", index=lab_opciones.index(m.get("Lab")) if m.get("Lab") in lab_opciones else None, placeholder="Seleccione...")
            if laboratorio == "OTRO": st.text_input("Especifique otro laboratorio:", key="k_otro_lab", value=m.get("Otro_Lab", ""))
        with c2:
        ...
        if resultado == "CON DESARROLLO/ POSITIVO":
            ...
            micro = st.selectbox("MICROORGANISMO AISLADO", microorganismos, key="k_micro", index=microorganismos.index(m.get("MicroOrg")) if m.get("MicroOrg") in microorganismos else None, placeholder="Seleccione...")
            if micro == "Otros": st.text_input("Especifique otro microorganismo:", key="k_otro_micro", value=m.get("Otro_Micro", ""))

            st.subheader("Prueba de Susceptibilidad")
            realizo_susp = st.radio("¿SE REALIZÓ PRUEBA DE SUSCEPTIBILIDAD ANTIMICROBIANA?", ["No", "Sí"], key="k_susp", index=["No", "Sí"].index(m.get("Susp", "No")), horizontal=True)
    ...
    if col_atras.button("⬅️ Atrás"): guardar(); st.session_state.pagina_actual = ORDEN[ORDEN.index(st.session_state.pagina_actual) - 1]; st.rerun()
    if col_guardar.button("Guardar registro y continuar"): guardar(); st.session_state.pagina_actual = ORDEN[ORDEN.index(st.session_state.pagina_actual) + 1]; st.rerun()
