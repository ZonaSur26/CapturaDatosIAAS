...
def render():
    ...
    se_periferica, se_cateter, se_punta = "No", "No", "No"
    
    if hemocultivo_its == "Sí":
        se_periferica = st.radio("SANGRE PERIFÉRICA", ["No", "Sí"], horizontal=True)
        se_cateter = st.radio("SANGRE POR CATETÉR CENTRAL", ["No", "Sí"], horizontal=True)
        se_punta = st.radio("PUNTA DE CATETÉR CENTRAL", ["No", "Sí"], horizontal=True)

    # --- 2. PREGUNTA GENERAL ---
    se_tomo_muestra = st.radio("¿SE TOMÓ MUESTRA PARA DIAGNÓSTICO MICROBIOLÓGICO?", ["No", "Sí"], index=None, horizontal=True)

    # El formulario se abre si se eligió Sí en ITS o Sí en la muestra general
    if se_tomo_muestra == "Sí" or hemocultivo_its == "Sí":
        ...
        if realizo_susp == "Sí":
            ...
            for ab in antibioticos:
                ...
                c1.write(ab)
                c2.radio(f"Res_{ab}", ["S", "I", "R", "ND"], key=f"res_{ab}", horizontal=True, label_visibility="collapsed")
        ...
        if resultado == "CON DESARROLLO/ POSITIVO":
            ...
            micro = st.selectbox("MICROORGANISMO AISLADO", microorganismos, index=None)
