...
def render():
    ...
    st.markdown("""
        <style>
        .fila-antibiotico { padding: 8px 10px; border-bottom: 1px solid #eee; display: flex; align-items: center; }
        .sombreado-base { background-color: #f9f9f9; } 
        .fila-activa { background-color: #e8f5e9 !important; border: 1px solid #4caf50 !important; border-radius: 5px; } 
        </style>
    ...
    if se_tomo_muestra == "Sí" or hemocultivo_its == "Sí":
        ...
        with c2:
            ...
            tipo_muestra = st.selectbox("TIPO DE MUESTRA", ["Orina de chorro medio", "Orina por puncion suprapubica", "Orina cateter vesical", "Sonda de entrada por salida", "Aspirado bronquial", "Aspirado traqueal", "Esputo inducido", "Esputo espontaneo", "Lavado bronco alveolar", "Biopsia transbronquial", "Biopsia pulmonar", "Biopsia pleural", "Sangre periferica", "Aspirado", "Biopsia", "Exudado faringeo", "Drenaje otico", "Sangre por cateter central", "Punta de cateter central", "Drenaje", "Liquido pericardico", "Heces", "Aspirado de absceso o secrecion de herida", "Hisopado de herida quirurgica +score Q", "Liquidos organicos", "Biopsia cuantitativa", "Aspirado de absceso", "Hisopado + score Q", "Liquido sinovial", "Sonicado de piezas protesicas para cultivo", "Hisopado conjuntival", "Raspado ocular", "Aspirado de humor vitreo", "Aspirado de humor acuoso", "Lavado ocular", "Liquido cefaloraquideo", "Liquido peritoneal", "Otro"], index=None, placeholder="Seleccione...")
            if tipo_muestra == "Otro": st.text_input("Especifique otro tipo de muestra:")

        # --- B. TÉCNICA Y RESULTADO ---
        st.selectbox("TÉCNICA PARA DIAGNÓSTICO MICROBIOLÓGICO", ["Bioquímicas manuales", "Inmunocromatografía", "Manuales API", "Semi-automatizados (AutoScan)", "VITEK (automatizada)", "MicroScan (automatizada)", "Aries Sensititre (automatizada)", "Phoenix (automatizada)", "Espectrometría de masas. MALDI-TOF", "PCR (moleculares)", "Sondas de hibridación (moleculares)"], index=None, placeholder="Seleccione...")
        ...
        if realizo_susp == "Sí":
            ...
            h1, h2, h3, h4 = st.columns([0.5, 2, 2.5, 1])
            h1.write("**Act**"); h2.write("**Antibiótico**"); h3.write("**Resultado**"); h4.write("**CMI**")
            
            for i, ab in enumerate(antibioticos):
                is_active = st.checkbox("", key=f"check_{ab}")
                ...
                col1.checkbox("", key=f"c_{ab}", value=is_active, label_visibility="collapsed")
                col2.markdown(f"**{ab}**")
