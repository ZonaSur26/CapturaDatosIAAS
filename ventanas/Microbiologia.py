...
def render():
    ...
    if (se_tomo_muestra == "Sí" or hemocultivo_its == "Sí") and hab_micro:
        ...
        with c1:
            ...
            if st.session_state.get("k_lab") == "OTRO": st.text_input("Especifique otro laboratorio:", key="k_otro_lab", value=m.get("Otro_Lab", ""))
        with c2:
        ...
        if resultado == "CON DESARROLLO/ POSITIVO":
            ...
            if st.session_state.get("k_micro") == "Otros": st.text_input("Especifique otro microorganismo:", key="k_otro_micro", value=m.get("Otro_Micro", ""))
            ...
            if realizo_susp == "Sí":
                ...
                for ab in antibioticos:
                    with st.container():
                        ...
                        if c1.checkbox(f"**{ab}**", key=f"check_{ab}", value=m.get(f"check_{ab}", False)):
                            c2.radio(f"Res_{ab}", ["S", "I", "R", "ND"], key=f"res_{ab}", index=["S", "I", "R", "ND"].index(m.get(f"res_{ab}", "ND")) if m.get(f"res_{ab}") else 3, horizontal=True, label_visibility="collapsed")
    ...
    def guardar():
        data = {
            "Hemo_ITS": st.session_state.k_hemo_its, "Tomada": st.session_state.k_tomo_muestra,
            "MicroOrg": st.session_state.get("k_micro", ""), "Susp": st.session_state.get("k_susp", "No")
        }
        for ab in ["AMPICILINA", "AMPICILINA-SULBACTAM", "ANFOTERICINA B", "ANIDULAFUNGINA", "AZTREONAM", "CASPOFUNGINA", "CEFAZOLINA", "CEFEPIME", "CEFOTAXIMA", "CEFOTETAN", "CEFOXITINA", "CEFTAROLINA", "CEFTAZIDIMA", "CEFTAZIDIMA-AVIBACTAM", "CEFTOLOZANE-TAZOBACTAM", "CEFTRIAXONA", "CIPROFLOXACINO", "CLINDAMICINA", "COLISTINA", "DAPTOMICINA", "ERITROMICINA", "ERTAPENEM", "FLUCONAZOL", "FOSFOMICINA", "GENTAMICINA", "IMIPENEM", "ITRACONAZOL", "LEVOFLOXACINO", "LINEZOLID", "MEROPENEM", "MICAFUNGINA", "NITROFURANTOINA", "OXACILINA", "PENICILINA", "PIPERACILINA-TAZOBACTAM", "POSACONAZOL", "RIFAMPICINA", "TETRACICLINA", "TIGECICLINA", "TRIMETOPRIM-SULFAMETOXAZOL", "VANCOMICINA", "VORICONAZOL"]:
