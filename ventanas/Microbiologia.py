...
def render():
    ...
    with st.container(border=True):
        ...
        # Despliegue EXCLUSIVO para hemocultivos
        if hemocultivo_its == "Sí":
            with st.container(border=True):
                st.markdown("**Origen del hemocultivo:**")
                c_hem1, c_hem2, c_hem3 = st.columns(3)
                sp_val = c_hem1.radio("SANGRE PERIFÉRICA", ["No", "Sí"], key="sp", index=buscar_idx(["No", "Sí"], m.get("sp", "No")), horizontal=True)
                scc_val = c_hem2.radio("SANGRE POR CATÉTER CENTRAL", ["No", "Sí"], key="scc", index=buscar_idx(["No", "Sí"], m.get("scc", "No")), horizontal=True)
                pcc_val = c_hem3.radio("PUNTA DE CATÉTER CENTRAL", ["No", "Sí"], key="pcc", index=buscar_idx(["No", "Sí"], m.get("pcc", "No")), horizontal=True)
        else:
            sp_val, scc_val, pcc_val = "NO", "NO", "NO"
    ...
    with st.container(border=True):
        ...
        f_toma, f_res, lab_sel, muestra_sel, tecnica_sel, resultado_sel, micro_sel, susp_sel = None, None, "", "", "", "", "", "No"

        # Despliegue EXCLUSIVO para diagnóstico microbiológico
        if se_tomo_muestra == "Sí":
            c1, c2 = st.columns(2)
            with c1:
                f_toma = st.date_input("FECHA DE TOMA", key="k_f_toma", value=m.get("Fecha_Toma") or None, format="DD/MM/YYYY")
                lab_opciones = ["De la unidad", "InDRE", "LESP", "LAVE", "PRIVADO/SUBROGADO", "OTRO"]
                lab_sel = st.selectbox("LABORATORIO", lab_opciones, key="k_lab", index=buscar_idx(lab_opciones, m.get("Lab")), placeholder="Seleccione...")
            
            with c2:
                f_res = st.date_input("FECHA DE RESULTADO", key="k_f_res", value=m.get("Fecha_Res") or None, format="DD/MM/YYYY")
                ...
                muestra_sel = st.selectbox("TIPO DE MUESTRA", muestras_opciones, key="k_muestra", index=buscar_idx(muestras_opciones, m.get("Muestra")), placeholder="Seleccione...")
            ...
            tecnica_sel = st.selectbox("TÉCNICA PARA DIAGNÓSTICO MICROBIOLÓGICO", tecnicas, key="k_tecnica", index=buscar_idx(tecnicas, m.get("Tecnica")), placeholder="Seleccione...")
    ...
    def guardar():
        ...
        data = {
            "Hemo_ITS": clean_val(hemocultivo_its), 
            "sp": clean_val(sp_val),
            "scc": clean_val(scc_val), 
            "pcc": clean_val(pcc_val),
            "Tomada": clean_val(se_tomo_muestra), 
            "Fecha_Toma": f_toma,
            "Fecha_Res": f_res,
            "Lab": clean_val(lab_sel), 
            "Muestra": clean_val(muestra_sel), 
            "Tecnica": clean_val(tecnica_sel),
            "Resultado": clean_val(resultado_sel),
            "MicroOrg": clean_val(micro_sel),
            "Susp": clean_val(susp_sel)
        }
